...
import InputDate from "../common/Inputs/Input/InputDate";
import {
...
import { updateOrgShortName } from "@/store/features/auth/authSlice";
...
function OrganizationProfileUpdate({ orgData, orgId }) {
  ...
  const [formData, setFormData] = useState({
    ...
    website_url: "",
    organization_code: orgShortName || decoded?.org_short_name || null,
  });
  // Location Hierarchy
    ...
    if (orgData) {
      ...
      setFormData({
        ...
        phone: orgData.organization?.mobile_no
          ...
          : "",
        phoneExist: orgData.organization?.mobile_no ? true : false,
        website_url: orgData.organization?.website_url,
        total_employee: orgData.organization?.total_employee,
      ...
      if (
        orgData &&
        orgData?.organization?.contact_type_org &&
        orgData?.organization?.contact_type_org.length > 0
      ) {
        setAddContactLinks(
          orgData?.organization?.contact_type_org.map((item) => ({
            uid: item?.id,
  ...
  // established data handler for change state value
  ...
  const addNewLink = () => {
    setAddContactLinks([
      ...addContactLinks,
      { id: null, uid: null, type_name: "", value: "" },
    ]);
  };
  ...
  const [updateOrgProfileInfo, { isLoading, error }] =
    useUpdateOrgProfileInfoMutation();
  ...
  const updateOrganizationHandler = async () => {
    const data = {
      ...
      total_employee: Number(formData?.total_employee),
      ...
      mobile:
        formData.phone && selectedCountry?.dial_code
      ...
      const checkData =
        organization && locationUpdateToggle && contact_type_list.length > 0
          ? {
              ...
              organization_contact: contact_type_list,
            }
          : organization && contact_type_list.length > 0
          ? { ...organization, organization_contact: contact_type_list }
          : { ...organization };
      ...
      if (formImg) {
        await uploadImageAndProcess({
          ...
          onSuccess: async (imageUrl) => {
            const response = await updateOrgProfileInfo({
              id: Number(orgId),
              data: {
                ...sanitizedData,
                organization: {
            ...
            // Update Redux store with new org short name
            if (response.data?.success && data.organization_code) {
              dispatch(updateOrgShortName(data.organization_code));
            }
          },
          onError: async (error) => {
            const response = await updateOrgProfileInfo({
            ...
            // Update Redux store with new org short name even on image error
            if (response.data?.success && data.organization_code) {
              dispatch(updateOrgShortName(data.organization_code));
            }
          },
      ...
      <div className="w-full scrollable">
        ...
        <div className="grid lg:grid-cols-2 md:grid-cols-1 grid-cols-1 gap-x-8 gap-y-4 mt-4 items-baseline">
          ...
          <InputDate
            ...
            error={
              errors && errors.established_date ? errors.established_date : null
            }
          />
          ...
          <div className="col-span-full">
            <InputWithLabel
              label={"Website"}
              placeholder={"Ex. https://organization.com"}
              type={"url"}
              name="website_url"
              handler={(e) => handleChange(e)}
          ...
          <div className="col-span-[70%]">
            <InputWithLabel
              ...
              defaultValue={orgShortName || decoded?.org_short_name}
              maxlength={4}
        ...
        <div className="w-full flex justify-end items-center mt-10">
          <button
            ...
            className="btn md:w-auto w-full bg-[#22252B] disabled:cursor-not-allowed disabled:opacity-50 py-[13px] px-[24px] font-bold text-lg rounded-[8px] text-white"
          >
