...
const getValueColor = (title) => {
  if (title === "Present") return "text-green-500";
  if (title === "Absent") return "text-red-500";
  if (title === "On Leave" || title === "Sick") return "text-orange-500";
  return "text-[#222]";
};

const ClassTopCard = ({ data }) => {
  return (
    <div className="lg:border-r-2 last:border-r-0  border-white  px-6  ">
      <h2
        className={`text-[48px] block pb-2 font-bold ${getValueColor(
          data.title
        )}`}
      >
        {data.value}
